"""Trader6 — Metals Momentum Options Engine
Strategy: Buy CALLS on golden cross, buy PUTS on death cross. Gold, Silver, Rare Earth.
Universe: GLD, GDX, NEM, GOLD, SLV, PAAS, AG, HL, MP, REMX, LAC
Exit: 60% profit target, 40% loss stop, or 21 DTE (whichever first).
"""
# ══════════════════════════════════════════════════════════════
...
POSITIONS_FILE = os.path.join(DATA_DIR, "trader6_positions.json")
os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(DATA_DIR, exist_ok=True)

PORT = deployer.get_port(6)
CLIENT_ID = 60
MONITOR_CLIENT_ID = 61

# Metals Momentum parameters
UNIVERSE = ["GLD", "GDX", "NEM", "GOLD", "SLV", "PAAS", "AG", "HL", "MP", "REMX", "LAC"]
CAPITAL = 15000  # $15k allocation
MAX_POSITIONS = 5
POSITION_SIZE = 600  # ~$600 per options position (leveraged)
MIN_SCORE = 3.0

# Options parameters
TARGET_DTE_MIN = 45  # Minimum days to expiration
TARGET_DTE_MAX = 90  # Maximum days to expiration
STRIKE_OFFSET = 0.02  # 2% OTM (slightly out of the money)
PROFIT_TARGET = 0.60  # Close at +60%
LOSS_STOP = -0.40  # Close at -40%
DTE_EXIT = 21  # Close if less than 21 DTE remaining


def log(msg):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[Trader6 {ts}] {msg}")
    with open(f"{LOG_DIR}/trader6.log", "a") as f:
        f.write(f"[{ts}] {msg}\n")
...
def get_technicals(symbol):
    ...
    try:
        ...
        return {
            "price": price, "ema50": ema50, "ema200": ema200, "sma50": sma50,
...
def score_bullish(tech):
    ...
    if tech["vol_current"] > tech["vol_avg"] * 1.5:
        score += 0.5
...
def find_option_contract(ib, symbol, price, direction="C"):
    ...
    # Strike: slightly OTM (calls above, puts below)
    if direction == "C":
...
def scan_and_enter():
    ...
    open_symbols = [(p["symbol"], p["direction"]) for p in open_positions]
...
def check_exits():
    ...
    for pos in open_positions:
        ...
        if pnl_pct >= PROFIT_TARGET:
            ...
            tech = get_technicals(symbol)
            if tech:
                if direction == "C" and tech["ema50"] < tech["ema200"]:
                    exit_reason = "Death cross reversal (close CALL)"
                elif direction == "P" and tech["ema50"] > tech["ema200"]:
                    exit_reason = "Golden cross reversal (close PUT)"
        ...
        if exit_reason:
            ...
            emoji = "🟢" if pnl >= 0 else "🔴"
            telegram.send(
                f"{emoji} *Metals Exit — {symbol} {expiry} ${strike}{direction}*\n\n"
...
if __name__ == "__main__":
    log("Trader6 — Metals Momentum OPTIONS Engine (Calls + Puts)")
