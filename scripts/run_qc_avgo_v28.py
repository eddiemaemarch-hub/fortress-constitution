"""Run AVGO v2.8 BASE backtest + walk-forward analysis on QuantConnect.

RESEARCH ONLY — does not modify any live trading code.

v2.8 BASE: NO golden cross trend adder. Pure cycle-low entry only.
Compare against v2.8+ (which got +501.5%, Sharpe 0.888, 18.8% DD, 106 orders).
...
ALGO_PATH = os.path.expanduser("~/rudy/quantconnect/AVGOCycleLowLeap_v28.py")
OUTPUT_PATH = os.path.expanduser("~/rudy/data/avgo_v28_backtest.json")
...
# v2.8+ reference results for comparison
V28PLUS_REFERENCE = {
    "total_return_pct": 501.5,
    "sharpe": 0.888,
...
def run_single_backtest(algo_code, window_name):
    ...
    project_name = f"AVGO_v28base_{window_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    log(f"\n{'='*60}")
    ...
    # Launch backtest
    bt_name = f"AVGO v2.8 BASE {window_name}"
...
def main():
    log("AVGO v2.8 BASE — QC BACKTEST + WALK-FORWARD ANALYSIS")
    log("NO TREND ADDER — Pure cycle-low entry only")
    ...
    # Build output
    output = {
        "strategy": "AVGO_v28_Base",
        "ticker": "AVGO",
        "description": "Broadcom v2.8 BASE Cycle-Low LEAP — NO trend adder. "
                       "Pure 200W SMA dip+reclaim entry with StochRSI filter. "
                       "Uses price/200W-SMA ratio instead of mNAV. "
                       "SPY > 200W SMA as market filter.",
        "version": "v2.8",
        "trend_adder": False,
        "timestamp": datetime.now().isoformat(),
        "windows": [],
        "walk_forward_metrics": wf_metrics,
        "v28plus_comparison": V28PLUS_REFERENCE,
        "adaptations": {
            ...
            "trend_adder": "DISABLED — v2.8 base strategy only",
        },
    }
    ...
    # Print comparison table: v2.8 BASE windows
    log(f"\n{'='*60}")
    log("v2.8 BASE RESULTS BY WINDOW")
    log(f"{'='*60}")
        ...
        if r and r.get("statistics"):
            ...
            for k, v in stats.items():
                ...
                if "drawdown" in k.lower():
                    dd = v
                if "total trades" in k.lower() or "total orders" in k.lower():
                    trades = v
                if "win rate" in k.lower():
                    wr = v
            log(f"{name:<8} {ret:<12} {sharpe:<10} {dd:<12} {trades:<8} {wr:<8}")
            if name == "FULL":
                full_stats = {"return": ret, "sharpe": sharpe, "drawdown": dd, "trades": trades}

    # Print v2.8 vs v2.8+ comparison
    log(f"\n{'='*60}")
    log("v2.8 BASE vs v2.8+ TREND ADDER COMPARISON (FULL PERIOD)")
    log(f"{'='*60}")
    log(f"{'Metric':<25} {'v2.8 BASE':<15} {'v2.8+ ADDER':<15}")
    log("-" * 55)
    log(f"{'Total Return':<25} {full_stats.get('return', '?'):<15} {'501.5%':<15}")
    log(f"{'Sharpe Ratio':<25} {full_stats.get('sharpe', '?'):<15} {'0.888':<15}")
    log(f"{'Max Drawdown':<25} {full_stats.get('drawdown', '?'):<15} {'18.8%':<15}")
    log(f"{'Total Orders':<25} {full_stats.get('trades', '?'):<15} {'106':<15}")
