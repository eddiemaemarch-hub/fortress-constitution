#!/usr/bin/env python3
"""Trader2 — MSTR $50 Put (Jan 2028) — Laddered Trailing Stop + Take Profit

Monitors the existing MSTR $50 Put position.
Activates laddered exit strategy once position is up +150%.
...
Position Details:
    ...
    Strike: $50
    Expiry: 2028-01-21
    Avg Cost: $12.53/share ($1,253.05 total)
    Qty: 1 contract (100 shares)

Ladder Strategy (activates at +150% gain):
    Tier 1: At +150% gain → Trail stop activated
    Tier 2: At +300% gain → Sell 25% + tighten trail
    Tier 3: At +500% gain → Sell 25% + tighten trail
    Tier 4: At +800% gain → Sell 25% + tighten trail
    Remaining 25%: Let ride with tight trailing stop from peak
...
Usage:
    python3 trader2_mstr_put.py              # Run as daemon
    python3 trader2_mstr_put.py --test       # Check position, no trades
    python3 trader2_mstr_put.py --status     # Show current state
"""
...
# ── Config ──
PORT = 7496
CLIENT_ID = 12  # Unique client ID (trader_v28 uses 10)
STATE_FILE = os.path.join(os.path.dirname(__file__), "..", "data", "trader2_state.json")
LOG_FILE = os.path.join(os.path.dirname(__file__), "..", "logs", "trader2.log")
...
# MARKDOWN = trend in our favor → wider trails, let puts run
# MARKUP = trend against us → tighter trails, protect gains faster
REGIME_TRAIL_ADJUST = {
...
RIGHT = "P"
EXPIRY = "20280121"
AVG_COST = 1253.05  # Total cost basis
QTY = 1  # Contracts

# Activation threshold
ACTIVATION_PCT = 150  # Strategy activates at 150% gain (puts spike fast, need early protection)

# Ladder tiers: (gain_pct_trigger, sell_fraction_of_remaining, trail_stop_pct_from_peak)
# sell_fraction is fraction of REMAINING position, not original
LADDER = [
    {"name": "Tier 1", "trigger_pct": 150,  "sell_frac": 0.0,  "trail_pct": 20},   # No sell, set 20% trail from peak
    {"name": "Tier 2", "trigger_pct": 300,  "sell_frac": 0.25, "trail_pct": 18},   # Sell 25%, tighten trail
    {"name": "Tier 3", "trigger_pct": 500,  "sell_frac": 0.33, "trail_pct": 15},   # Sell 33% of remaining
    {"name": "Tier 4", "trigger_pct": 800,  "sell_frac": 0.50, "trail_pct": 12},   # Sell 50% of remaining
]

# Final runner: 15% trailing stop on whatever is left
RUNNER_TRAIL_PCT = 15

# ── LEAP Expiry Extension Protocol ──
# Propose rolling to a later expiry when approaching expiry in a flat market.
# "Flat" = puts have not activated (underlying hasn't crashed enough).
# Rolling same strike forward preserves the bearish thesis with more time.
EXPIRY_ROLL_WARNING_DAYS = 180   # 6 months out — early warning
EXPIRY_ROLL_URGENT_DAYS  = 90    # 3 months out — urgent
NEXT_EXPIRY = "20300117"         # MSTR $50P Jan 2030 (2-yr forward roll)
...
def get_regime():
    ...
    Returns dict with regime, confidence, trail_adjust, phase."""
    try:
...
class Trader2:
    ...
    def check_position(self):
        """Main evaluation loop — check position and apply ladder logic."""
        ...
        log(f"MSTR $50P Jan28 | Mid: ${mid_price:.2f} | Value: ${market_value:.2f} | "
        ...
        if not self.state["activated"]:
            if gain_pct >= ACTIVATION_PCT:
                ...
                # Set initial trail stop (regime-adjusted)
                tier = LADDER[0]
        ...
        if self.state["activated"]:
            ...
            if market_value > self.state["peak_value"]:
                self.state["peak_value"] = market_value
                self.state["peak_gain_pct"] = gain_pct
                # Recalculate trail stop from new peak (regime-adjusted)
                trail_pct = self.state["trail_stop_pct"]
                effective_trail = max(5, trail_pct + trail_adj)  # Floor at 5%
    ...
    def _trigger_tier(self, tier, market_value, gain_pct, mid_price):
        """Handle hitting a new ladder tier."""
        sell_qty = 0
        if tier["sell_frac"] > 0:
            # Calculate contracts to sell (for single contract, this means we track fractionally
            # but execute whole contracts only)
            sell_qty = max(0, int(self.state["contracts_remaining"] * tier["sell_frac"]))
            # For single contract positions, we can't sell fractions
            # Instead we'll note it and sell the full contract at the final tier
            if sell_qty == 0 and tier["sell_frac"] > 0 and self.state["contracts_remaining"] >= 1:
                # Single contract — can't split. Just tighten the trail and note it.
                log(f"Single contract — can't split at {tier['name']}. Tightening trail only.")
                sell_qty = 0

        # Tighten trail stop (regime-adjusted)
        regime_info = get_regime()
        ...
        if sell_qty > 0:
            ...
            if tier["sell_frac"] > 0:
                msg += f"⚠️ Single contract — can't split. Trail tightened instead.\n"

        msg += (
            f"Trail: {tier['trail_pct']}% from peak\n"
    ...
    def _trigger_trail_stop(self, market_value, gain_pct, mid_price):
        """Trail stop hit — send HITL approval to sell everything remaining."""
    ...
    def execute_sell(self, qty):
        """Execute a sell order via IBKR. Called after HITL approval."""
        if self.test_mode:
            log(f"TEST MODE — would sell {qty} MSTR $50P Jan28")
            return True
        ...
        if trade.orderStatus.status == "Filled":
            ...
            send_telegram(f"🔴 *TRADER2: SELL FAILED*\nStatus: {trade.orderStatus.status}")
            return False

    def approve_pending_sell(self):
        """Called via API/MCP when user approves a pending sell."""
    ...
    def reject_pending_sell(self):
        """Called via API/MCP when user rejects a pending sell."""
        tier = self.state.get("pending_sell", {}).get("tier", "unknown")
        self.state["pending_sell"] = None
        save_state(self.state)
        send_telegram(f"❌ *TRADER2: Sell rejected* ({tier})\nPosition held. Trail stop still active.")
    ...
    def _check_expiry_extension(self, gain_pct, market_value, mid_price):
        ...
        # Only roll if position is flat — if it's activated and printing, let it run
        is_flat = (not self.state.get("activated", False)
                   and gain_pct < (ACTIVATION_PCT * 0.5))
        if not is_flat:
            return

        # Don't double-propose if one is already pending
        ...
        send_telegram(
            f"{urgency} *TRADER2: LEAP EXPIRY EXTENSION*\n"
            f"MSTR ${STRIKE:.0f} Put — flat market, time decay risk\n"
            f"━━━━━━━━━━━━━━━━\n"
    ...
    def approve_expiry_roll(self):
        ...
        # Bridge into execute_roll() infrastructure
    ...
    def reject_expiry_roll(self):
        ...
        save_state(self.state)
        send_telegram(
            f"❌ *TRADER2: Expiry roll rejected*\n"
    ...
    def propose_strike_roll(self, new_strike, new_expiry=None):
        ...
        send_telegram(
            f"🔄 *TRADER2: STRIKE ROLL PROPOSED*\n"
            f"MSTR Put\n"
            f"━━━━━━━━━━━━━━━━\n"
    ...
    def execute_roll(self):
        ...
        if sell_trade.orderStatus.status != "Filled":
            send_telegram(f"🔴 *TRADER2: ROLL FAILED — Could not sell old contract*\n{sell_trade.orderStatus.status}")
        ...
        if buy_trade.orderStatus.status != "Filled":
            send_telegram(
                f"🔴 *TRADER2: ROLL PARTIAL — Sold old but could not buy new!*\n"
        ...
        # Reset peak/trail since the position changed
    ...
    def reject_roll(self):
        self.state["pending_roll"] = None
        save_state(self.state)
        send_telegram(f"❌ *TRADER2: Roll rejected*\nKeeping current ${STRIKE} strike.")
        return {"status": "rejected"}
    ...
    def show_status(self):
        ...
        s = self.state
        print(f"\n{'='*50}")
        print(f"TRADER2 — MSTR $50 Put Jan 2028")
        print(f"{'='*50}")
    ...
    def run_daemon(self):
        """Run as a scheduled daemon — checks every 5 minutes during market hours."""
        log(f"\n{'='*60}")
        log(f"TRADER2 — MSTR $50 Put Ladder Monitor STARTED")
        log(f"Activation: +{ACTIVATION_PCT}% gain")
        log(f"Cost Basis: ${AVG_COST:.2f}")
        log(f"{'='*60}\n")

        send_telegram(
            f"🤖 *Trader2 Daemon Started*\n"
            f"MSTR $50 Put Jan 2028\n"
            f"Monitoring for +{ACTIVATION_PCT}% gain to activate ladder\n"
            f"Checking every 5 min during market hours"
        )

        # Check every 5 minutes during market hours (9:30 AM - 4:00 PM ET)
...
def main():
    parser = argparse.ArgumentParser(description="Trader2 — MSTR Put Ladder Monitor")
    ...
    if not (args.test or args.status or args.check):
        lockfile = os.path.join(os.path.dirname(__file__), "..", "data", "trader2.pid")
    ...
    trader = Trader2(test_mode=args.test)
