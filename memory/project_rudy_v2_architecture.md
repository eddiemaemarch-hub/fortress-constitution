...
- **Bottom/ATH predictions (April 8, 2026):** Claude extrapolation: ~$34K bottom / ~$204K next ATH. Commander prediction: ~$35K (72% drop). Camirand: ~$56K. H&S chart: $48-52K. All scenarios: v2.8+ fires on 200W SMA reclaim regardless of bottom depth.
...
- **MA Bounce / Breakout / Slope filters — ALL REJECTED (March 26, 2026):** Tested via `backtest_ma_bounce_breakout_v28plus.py`. Bounce filter, Golden Cross (50W > 200W), slope filter, and combined variant all generated zero trades in isolated walk-forward and stress windows. v2.8+ signals are rare by design — additive filters starve the already-sparse signal. Baseline (4 trades, 3.25 PF, 0.28 Sharpe) remains best configuration. **v2.8+ logic is FROZEN. Do not modify.**
- **EMA Ribbon (21/34/144) + Stochastic (7,3,3) — REJECTED (March 26, 2026):** Tested via `backtest_ema_stoch_reversal.py`. Standalone: 4 trades, all losers, -30% return, negative Sharpe — does not work on BTC weekly for MSTR LEAP entries. As v2.8+ filter: zero trades — signals never fire within ±4 weeks of a v2.8+ signal, completely incompatible timing. Zero trades across all 20 WF windows and 8 stress scenarios. Not a DEAP candidate. v2.8+ baseline untouchable.
