...
# MSTR Cycle-Low LEAP Entry Strategy Backtest v2.6 LOOSENED
#
# v2.6 CHANGES from v2.5 (catch more mid-cycle moves):
#   1. Green weeks threshold: 2 → 1 (faster entry after 200W reclaim)
#   2. Premium cap: 1.5x → 2.0x (allows entry at slightly higher premiums)
#   3. Max entries per cycle: 1 → 2 (can re-enter after stop-out within same cycle)
#
# ENTRY: 200W SMA dip+reclaim pattern (price dips below 200W, then reclaims with 1+ green week)
# FILTERS: BTC > 200W MA, StochRSI < 70, premium not contracting, no MACD bearish div, premium <= 2.0x
#          ATR volatility filter (only enter when ATR < 1.5x its 20-day average = quiet market)
# RE-ENTRY: Allowed after stop-out, max 2 entries per cycle
...
class MSTRCycleLowLeap(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.green_weeks_threshold = 1       # v2.6: 2→1 green week (faster entry after reclaim)
        ...
        self.premium_cap = 2.0               # v2.6: 1.5x→2.0x (allows mid-cycle entries at higher premiums)
        ...
        # ── State: Cycle tracking (v2.6: max 2 entries per cycle) ──
        self.already_entered_this_cycle = False
        self.cycle_entry_count = 0
        self.max_entries_per_cycle = 2
    ...
    def OnTradeBar(self):
        ...
        # ── Entry Filter 6: Max 2 entries per cycle (v2.6 loosened from 1) ──
        cycle_ok = self.cycle_entry_count < self.max_entries_per_cycle
        ...
        if all_filters and not self.Portfolio["MSTR"].Invested and not self.first_entry_done:
            ...
            if qty > 0:
                ...
                self.cycle_entry_count += 1
                if self.cycle_entry_count >= self.max_entries_per_cycle:
                    self.already_entered_this_cycle = True

                self.Log(f"ENTRY 2/2: {self.Time.strftime('%Y-%m-%d')} @ ${mstr_price:.2f} | "
                         f"Avg=${self.entry_price:.2f} | Qty={qty} | Total={total_qty} | CycleEntry={self.cycle_entry_count}")
    ...
    def RecordExit(self, reason, price, stock_gain, leap_gain):
        ...
        self.second_entry_done = False
        self.euphoria_sell_done = False
        # v2.6: Allow re-entry after stop-out if under max_entries_per_cycle
        if self.cycle_entry_count < self.max_entries_per_cycle:
            self.already_entered_this_cycle = False
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.Log("=" * 60)
        self.Log(f"MSTR CYCLE-LOW LEAP v2.6 LOOSENED ({res_name})")
        self.Log("=" * 60)
