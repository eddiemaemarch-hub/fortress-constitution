...
# MSTR Cycle-Low LEAP Entry Strategy Backtest v2.7 DIAMOND HANDS
#
# v2.7 CHANGES from v2.5 (ride the full cycle, keep moonshot alive):
#   1. Profit takes: 25% → 10% at each tier (keep 60%+ of position for moonshot)
#   2. Profit tiers pushed higher: 10x/20x/50x/100x (was 5x/10x/20x/50x)
#   3. Trail starts at 500% not 300% — more room to grow before trail attaches
#   4. Wider trails: 40%/35%/30%/25%/15% (was 30%/25%/20%/15%/10%)
#   5. Floor deactivation at 500% (was 300%) — more breathing room
#   6. Removed 20x target exit — let it ride past 20x
#   7. Euphoria sell reduced: 25% → 15%
#
# GOAL: Enter at cycle low, hold 60%+ of position through the ENTIRE bull run
#        Only trim small amounts at massive milestones
#        Catch $115 → $2000+ type moves without selling most at $200-$460
#
# ENTRY: Same as v2.5 (200W dip+reclaim, all filters unchanged)
# LEAP Multiplier: Conservative staggered 7.5x (same as v2.5)
...
class MSTRCycleLowLeap(QCAlgorithm):
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.Log("=" * 60)
        self.Log(f"MSTR CYCLE-LOW LEAP v2.7 DIAMOND HANDS ({res_name})")
        self.Log("=" * 60)
