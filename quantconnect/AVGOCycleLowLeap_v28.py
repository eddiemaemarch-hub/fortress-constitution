...
# AVGO Cycle-Low LEAP Entry Strategy Backtest v2.8 BASE
#
...
class AVGOCycleLowLeap(QCAlgorithm):
    ...
    def OnEndOfAlgorithm(self):
        self.Log("=" * 60)
        self.Log("AVGO CYCLE-LOW LEAP v2.8 BASE (Daily) — NO TREND ADDER")
        self.Log("=" * 60)
        self.Log(f"Weeks: {self.week_count} | 200W SMA: {'Yes' if self.avgo_200w_sma else 'No'}")
        self.Log(f"Base Trades: {len(self.trade_log)}")
        ...
        for i, t in enumerate(self.trade_log):
            ...
            self.Log(f"Trade {i+1}: {entry_str} -> {exit_str} | "
