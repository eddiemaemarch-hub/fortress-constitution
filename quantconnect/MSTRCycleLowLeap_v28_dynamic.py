...
# MSTR Cycle-Low LEAP Entry Strategy Backtest v2.8 DYNAMIC BLEND
#
# v2.8 CHANGES from v2.7 Diamond Hands:
#   1. DYNAMIC LEAP multiplier based on mNAV premium at time of evaluation
#      - Premium < 0.8x (discount):  60% deep ITM(6x) + 40% ATM(12x) = 8.4x
#      - Premium 0.8-1.2x (fair):    50% deep ITM(5x) + 50% near-ATM(10x) = 7.5x
#      - Premium 1.2-1.5x (elevated):60% deep ITM(4x) + 40% near-ATM(8x) = 5.6x
#      - Premium > 1.5x (euphoric):  70% deep ITM(3x) + 30% OTM(6x) = 3.9x
#   2. Rationale: When premium is LOW, IV is low → options cheap → more leverage.
#      When premium is HIGH, IV is elevated → options expensive → less leverage.
#      Shifting toward deep ITM at high premium = more conservative = smart.
#   3. All other v2.7 Diamond Hands features preserved (wider trails, small PTs, etc.)
#
# REAL-WORLD LEAP BLEND (what you'd actually buy):
#   LOW PREMIUM (<0.8x):  60% deep ITM calls (~0.85 delta, ~6x) + 40% ATM calls (~0.50 delta, ~12x)
#   FAIR VALUE (0.8-1.2x): 50% deep ITM calls (~0.80 delta, ~5x) + 50% near-ATM calls (~0.50 delta, ~10x)
#   ELEVATED (1.2-1.5x):  60% deep ITM calls (~0.75 delta, ~4x) + 40% near-ATM calls (~0.45 delta, ~8x)
#   EUPHORIC (>1.5x):     70% deep ITM calls (~0.70 delta, ~3x) + 30% OTM calls (~0.40 delta, ~6x)
#
# ENTRY: Same as v2.5 (200W dip+reclaim, all filters unchanged)
# LEAP Multiplier: DYNAMIC premium-based blend (3.9x to 8.4x range)
# Trail: 5-tier laddered with WIDER trails (40%/35%/30%/25%/15% at 5x/10x/20x/50x/100x)
# Profit Taking: 10% partial sells at 10x/20x/50x/100x LEAP-equiv
# 35% initial floor, -35% panic floor, 50/50 scale-in, risk capital 25%
#
...
class MSTRCycleLowLeap(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # Start at 5x (not 3x), much wider trails to let position breathe
        self.ladder_tiers = [
            (10000, 15.0),  # 100x+ -> 15% trail (tightest, locking massive gains)
            (5000,  25.0),  # 50x+  -> 25% trail
            (2000,  30.0),  # 20x+  -> 30% trail
            (1000,  35.0),  # 10x+  -> 35% trail
            (500,   40.0),  # 5x+   -> 40% trail (widest, let it ride)
        ]

        # ── v2.7 DIAMOND HANDS: Small Profit Takes at Higher Tiers ──
        # Only trim 10% at each milestone — keep 60%+ for the moonshot
        self.profit_tiers = [
            (1000,  0.10),  # PT1: 10x -> sell 10% (lock seed money)
            (2000,  0.10),  # PT2: 20x -> sell 10%
            (5000,  0.10),  # PT3: 50x -> sell 10%
            (10000, 0.10),  # PT4: 100x -> sell 10%
        ]

        # ── Risk Parameters ──
        self.max_hold_bars = 567              # ~27 months daily
        self.target_mult = 200.0              # v2.7: effectively disabled — let it ride
        ...
        self.floor_deactivate_leap_gain = 500 # v2.7: Floor deactivates above +500% LEAP (5x) — more room to grow
        self.panic_floor_pct = -35.0          # -35% LEAP P&L panic exit (losers only)
        self.euphoria_premium = 3.5           # v2.7: Sell 15% (not 25%) if premium > 3.5x and profitable
    ...
    def GetDynamicLeapMultiplier(self, premium):
        ...
        """
        if premium < 0.8:
            # Discount to NAV — LEAPs are cheap, go aggressive
            # 60% deep ITM (~0.85 delta, ~6x) + 40% ATM (~0.50 delta, ~12x)
            return 0.60 * 6.0 + 0.40 * 12.0  # = 8.4
        elif premium < 1.2:
            # Fair value — standard blend (same as v2.5/v2.7)
            # 50% deep ITM (~0.80 delta, ~5x) + 50% near-ATM (~0.50 delta, ~10x)
            return 0.50 * 5.0 + 0.50 * 10.0  # = 7.5
        elif premium <= 1.5:
            # Elevated — shift toward safer deep ITM
            # 60% deep ITM (~0.75 delta, ~4x) + 40% near-ATM (~0.45 delta, ~8x)
            return 0.60 * 4.0 + 0.40 * 8.0  # = 5.6
        else:
            # Euphoric premium — very conservative, mostly deep ITM
            # 70% deep ITM (~0.70 delta, ~3x) + 30% OTM (~0.40 delta, ~6x)
    ...
    def OnEndOfAlgorithm(self):
        ...
        self.Log("=" * 60)
        self.Log(f"MSTR CYCLE-LOW LEAP v2.8 DYNAMIC BLEND ({res_name})")
        self.Log("=" * 60)
